#!/usr/bin/env python3
"""
Visualization script for comparing Qwen LLM performance between Mali G710 and Tesla GPUs.
...
def parse_llamacpp_benchmark(filepath):
    ...
    try:
        ...
        for line in lines:
            ...
            if in_table and '|' in line and 'qwen' in line.lower():
                # Parse the benchmark line
                # Example: | qwen2 1B Q4_K - Medium | 373.71 MiB | 494.03 M | Vulkan | 99 | pp512 | 143.93 ± 1.17 |
...
def plot_qwen_comparison(mali_data, tesla_data):
    """Create comparison plot for Qwen LLM performance."""
    fig, ax = plt.subplots(1, 1, figsize=(10, 7))
    
    # Build title with model size (only once since both use same model)
    title = 'Qwen LLM Performance'
    model_size = None
    ...
    for i, (mali_val, tesla_val) in enumerate(zip(mali_means, tesla_means)):
        if mali_val > 0:
            # Position text above the bar (1.15x for log scale spacing)
...
def main():
    ...
    # Parse Mali GPU data
    mali_file = base_dir / 'fold' / 'qwen_0.5b_fold.txt'
    mali_data = None
    if mali_file.exists():
        ...
        if mali_data:
            ...
            for test, values in mali_data['tests'].items():
                print(f"    {test}: {values['mean']:.2f} ± {values['uncertainty']:.2f} t/s")
        else:
    ...
    tesla_file = base_dir / 'tesla' / 'llamacpp_bench_qwen_0.5b.txt'
    tesla_data = None
    if tesla_file.exists():
        ...
        if tesla_data:
            ...
            for test, values in tesla_data['tests'].items():
                print(f"    {test}: {values['mean']:.2f} ± {values['uncertainty']:.2f} t/s")
        else:
    ...
    fig = plot_qwen_comparison(mali_data, tesla_data)
    
    # Save plot
    output_file = base_dir / 'qwen_performance_comparison.png'
